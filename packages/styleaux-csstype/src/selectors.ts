...
export let getPseudos = () => {
  ...
  for (const selector in selectors) {
    ...
    while (
      (match = REGEX_ADVANCED_PSEUDO_SELECTOR.exec(selectors[selector].syntax))
    ) {
      advanced = addType(advanced, {
        type: Type.StringLiteral,
        literal: match[1],
      });
