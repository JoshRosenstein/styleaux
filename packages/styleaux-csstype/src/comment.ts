  ...
  if (typeof data.initial === 'string') {
    if (data.initial in l10n) {
      if (typeof l10n[data.initial]['en-US'] === 'string') {
        rows.push(
          `**Initial value**: ${formatL10n(l10n[data.initial]['en-US'])}`,
          BLANK_ROW,
        );
      }
    } else {
...
function getCompatRows(compatibilityData: MDN.CompatData) {
  ...
  const urls = compats.reduce<string[]>(
    (list, { mdn_url }) =>
      mdn_url && !list.includes(mdn_url) ? [...list, mdn_url] : list,
    [],
  );
...
function supportVersion(
  supports: MDN.Support | MDN.Support[] | undefined,
): string[] {
  supports = supports
    ? (Array.isArray(supports) ? supports : [supports]).reverse()
    : [];

  // Ignore versions hidden under flags
  supports = supports.filter(({ flags }) => !flags);

  const supportsVersions = supports.filter(
    ({ version_added }) => typeof version_added === 'string',
  );

  if (supportsVersions.length > 0) {
    // Find lowest version of standard implementation
    const supportsStandard = supportsVersions.reduce<MDN.Support | null>(
      (previous, current) => {
        if (!current.prefix && !current.alternative_name) {
          if (
            !previous ||
            (previous.version_added !== true &&
              current.version_added === true) ||
    ...
    const supportsPrefixed = supportsVersions.reduce<MDN.Support | null>(
      (previous, current) => {
        if (
          ...
          (!supportsStandard ||
            (supportsStandard.version_added !== true &&
              current.version_added === true) ||
            typeof supportsStandard.version_added !== 'string' ||
            typeof current.version_added !== 'string' ||
            versionDiff(supportsStandard.version_added, current.version_added) <
              0)
        ) {
          if (
            !previous ||
    ...
    if (supportsPrefixed) {
      version.push(
        (supportsStandard
          ? (supportsPrefixed.version_added as string)
          : `**${supportsPrefixed.version_added}**`) +
    ...
    const supportYes = supports.find(
      ({ version_added }) => version_added === true,
    );
    if (supportYes) {
      return ['Yes'];
    }

    const supportsNo = supports.find(
      ({ version_added }) => version_added === false,
    );
    if (supportsNo) {
      return ['No'];
    }
