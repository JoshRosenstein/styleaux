...
const COMPACTDATA=path.join(LERNAROOT,`mdn-browser-compat-data/css/properties`)
...
const types = fs
  .readFileSync(
    path.resolve(
      process.cwd(),
      '../../node_modules/@roseys/csstype/index.d.ts',
    ),
  )
  .toString()
  .split('\n')
...
async function writeUtilFileAsync(data: IUtilData) {
  ...
  const source = utilTemplate
    ...
    .replace(PROPERTY_NAME_CAP_REGEX, data.propertyName.toUpperCase())
      ...
      acc.then(async data => {
        const dataWithComments = await Promise.all(
          item.map(async i => {
            ...
            const comment = await scrapeSummary(i.url)
...
async function generateUtils() {
  const utilsToGenerate = Object.keys(cssProperties)
    .map(key => {
      ...
      let url = prop.hasOwnProperty('mdn_url') ? (prop as any).mdn_url : 'https://developer.mozilla.org/en-US/docs/Web/CSS/' + key
...
status='nonstandard'
    ...
    .filter((x)=>{
        return x && !['obsolete','nonstandard','deprecated'].includes(x.status)}
    )
