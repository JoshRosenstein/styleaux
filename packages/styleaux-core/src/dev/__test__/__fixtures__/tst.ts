...
const mg = style<{m: SpaceKeys}, IthemeS, IBreakPoints>({
  cssProp: 'margin',
  prop: 'm',
  themeProp: 'colors',
})

export function createstyle<DT extends {} = never, DB extends {} = never>(
  transformer: Function,
) {
  return function style<P, T extends {} = DT, B extends {} = DB>({
    cssProp,
    prop,
    themeProp,
  }: IStyleOptions<P, T>) {
...
const style2 = createstyle<IthemeS, IBreakPoints>(a => 1)

const mg2 = style2<{m: keyof IthemeS['colors']}>({
  cssProp: 'margin',
  prop: 'm',
  themeProp: 'colors',
})

const mg22 = mg2({m: 'secondary.dark'})
