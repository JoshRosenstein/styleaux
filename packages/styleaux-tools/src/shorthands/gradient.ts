...
const linearHelper = (gradientName: string) =>
  ((...args: any[]) => {
    ...
    return `${gradientName}(${commaSeparatedList(
      direction,
      ...steps.map(({ position, color }) =>
        spaceSeparatedList(color, position),
      ),
    )})`;
  }) as {
    (props: GradientLinearProps): string;
    (
      direction: GradientLinearProps['direction'],
      ...steps: GradientLinearProps['steps']
    ): string;
  };
...
const GradientradialHelper = (gradientName: string) =>
  ((...args: any[]) => {
    const steps: GradientStep[] =
      args.length === 1 && typeof args[0].steps !== 'undefined'
        ? args[0].steps
        : args;
...
export const repeatingRadial = GradientradialHelper(
